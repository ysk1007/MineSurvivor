...
namespace VinToolsEditor.BetterRuleTiles
{
    public class BetterRuleTileEditor : EditorWindow
    {
        ...
        private Texture2D t_tileTex_Delete;
        private Texture2D t_tileTex_Ignore;
        private Texture2D t_tileTex_Empty;
        private Texture2D t_tileTex_NotSame;
        private Texture2D t_tileTex_Any;

        private Texture2D t_backgroundTexture;
        private Texture2D t_toolbarBackgroundTexture;
        private Texture2D t_highlightTexture;

        private Texture2D t_emptyTexture;
        private Texture2D t_defaultButtonTexture;
        private Texture2D t_hoverButtonTexture;
        private Texture2D t_activeButtonTexture;

        private Texture2D t_windowBorderTexture;
        private Texture2D t_fieldBoxTexture;

        private Texture2D t_toolTex_Brush;
        private Texture2D t_toolTex_Pick;
        private Texture2D t_toolTex_Erase;
        private Texture2D t_toolTex_Move;
        private Texture2D t_toolTex_Select;

        private Texture2D t_actionTex_DeleteSelection;
        private Texture2D t_actionTex_Undo;
        private Texture2D t_actionTex_Redo;

        private Texture2D t_optionTex_Export;
        private Texture2D t_optionTex_LockWindows;
        private Texture2D t_optionTex_HideSprites;
        private Texture2D t_optionTex_Ruler;
        private Texture2D t_optionTex_Settings;
        private Texture2D t_optionTex_helpIcon;
        ...
        public void SetupGrid() => _grid = new EditorGridBase(this, false);
        ...
        void HandleTools()
        {
            ...
            switch (Event.current.keyCode)
            {
                ...
                case KeyCode.Escape: _hasSelection = false; break;
            }
        ...
        private GUIWindow[] _windows = new GUIWindow[5];
        ...
        private void SetUpWindows()
        {
            ...
            _window_TileInfo = new GUIWindow(GUIWindow.WindowLayout.BottomRightCorner, new RectOffset(4, 4, 30, 4), new Vector2(300, 350), TileInfoWindow, new GUIContent(""), false, true);
            _window_SettingsWindow = new GUIWindow(new Rect(0, 0, 250, 50), SettingsWindow, new GUIContent(""), false, true, true);
        ...
        private void DisplayWindows()
        {
            ...
            _window_TileInfo.visible = _selectedDrawerTile > 0;

            BeginWindows();
            for (int i = 0; i < _windows.Length; i++) _windows[i].Show(i, position);
            EndWindows();
        }
        ...
        public bool IsMouseOverWindow()
        {
            ...
            for (int i = 0; i < _windows.Length; i++) if (_windows[i].rectangle.Contains(Event.current.mousePosition) && _windows[i].visible) return true;
            return false;
        }
        #endregion
        ...
        /*void DebugWindow(int windowID)
        {
            ...
            EditorGUILayout.LabelField("Opened container", $"\"{_file.name}\"");
            EditorGUILayout.LabelField("Number of grid elements", _file.Grid.Count.ToString());
            EditorGUILayout.LabelField("Number of tiles", _file.Tiles.Count.ToString());

            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Mouse Position ", Event.current.mousePosition.ToString());
            EditorGUILayout.LabelField("Start position", d_currentGridStart.ToString());
            EditorGUILayout.LabelField("Grid Origin", d_currentGridOrigin.ToString());
            EditorGUILayout.LabelField("Cells", d_currentGridCellAmount.ToString());
            EditorGUILayout.LabelField("Cell size", d_currentGridCellSize.ToString());
            EditorGUILayout.LabelField("Offset", d_currentGridOffset.ToString());
            EditorGUILayout.LabelField("Scroll", _zoomScroll.ToString());
            EditorGUILayout.LabelField("Zoom", _zoomAmount.ToString());

            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Selected tool", _selectedTool.ToString());
            EditorGUILayout.LabelField("Selected tile", _selectedDrawerTile.ToString());
        ...
        void ToolbarWindow(int WindowID)
        {
            ...
            //divider
            GUI.DrawTexture(new Rect(6, 5, 1, 16), t_defaultButtonTexture);
            GUI.DrawTexture(new Rect(9, 5, 1, 16), t_defaultButtonTexture);
            //toolbar
            _selectedTool = GUI.Toolbar(new Rect(17, 3, 198 - 33, 20), _selectedTool, _toolbarTextures, _toolbarButtonStyle);
            //divider
            GUI.DrawTexture(new Rect(188, 5, 1, 16), t_defaultButtonTexture);
            GUI.DrawTexture(new Rect(191, 5, 1, 16), t_defaultButtonTexture);
            //actions
            if (GUI.Button(new Rect(199, 3, 32, 20), new GUIContent(t_actionTex_Undo, "Undo (Ctrl + Z)"), _toolbarButtonStyle)) Undo.PerformUndo();
            if (GUI.Button(new Rect(235, 3, 32, 20), new GUIContent(t_actionTex_Redo, "Redo (Ctrl + Y)"), _toolbarButtonStyle)) Undo.PerformRedo();
            if (GUI.Button(new Rect(271, 3, 32, 20), new GUIContent(t_actionTex_DeleteSelection, "Delete selected area (Delete)"), _toolbarButtonStyle)) DeleteSelection();


            //MIDDLE


            if (GUI.Button(new Rect(position.width - 303 + 34, 3, 32, 20), new GUIContent(t_optionTex_helpIcon, "Open documentation (opens browser)"), _toolbarButtonStyle)) Application.OpenURL("https://docs.vinark.dev/#/./better-rule-tiles/index");
            //divider
            GUI.DrawTexture(new Rect(position.width - 230, 5, 1, 16), t_defaultButtonTexture);
            GUI.DrawTexture(new Rect(position.width - 227, 5, 1, 16), t_defaultButtonTexture);
            //settings
            _showRuler = GUI.Toggle(new Rect(position.width - 219, 3, 32, 20), _showRuler, new GUIContent(t_optionTex_Ruler, "Show the grid coordinates on the edge of the screen"), _toolbarButtonStyle);
            _lockWindows = GUI.Toggle(new Rect(position.width - 183, 3, 32, 20), _lockWindows, new GUIContent(t_optionTex_LockWindows, "Snap windows to the corners and lock them"), _toolbarButtonStyle);
            _hideSprites = GUI.Toggle(new Rect(position.width - 147, 3, 32, 20), _hideSprites, new GUIContent(t_optionTex_HideSprites, "Hide sprites to see tiles below them"), _toolbarButtonStyle);
            //more settings
            _window_SettingsWindow.visible = GUI.Toggle(new Rect(position.width - 111, 3, 38, 20), _window_SettingsWindow.visible, new GUIContent(t_optionTex_Settings, "Other Settings"), _toolbarButtonStyle);
            _window_SettingsWindow.position = new Vector2(position.width - 111 + 38 - _window_SettingsWindow.width, 23);
            //divider
            GUI.DrawTexture(new Rect(position.width - 66, 5, 1, 16), t_defaultButtonTexture);
            GUI.DrawTexture(new Rect(position.width - 63, 5, 1, 16), t_defaultButtonTexture);
            //export button
            _window_ExportWindow.visible = GUI.Toggle(new Rect(position.width - 55, 3, 38, 20), _window_ExportWindow.visible, new GUIContent(t_optionTex_Export, "Export options"), _toolbarButtonStyle);
            _window_ExportWindow.position = new Vector2(position.width - 55 + 38 - _window_ExportWindow.width, 23);
            //divider
            GUI.DrawTexture(new Rect(position.width - 10, 5, 1, 16), t_defaultButtonTexture);
            GUI.DrawTexture(new Rect(position.width - 7, 5, 1, 16), t_defaultButtonTexture);
        }
        #endregion
        ...
        void TileDrawerWindow(int windowID)
        {
            ...
            _windows[windowID].width = _drawerSize * 70 + 80 + 19;
        ...
        void SettingsWindow(int windowID)
        {
            ...
            //
            EditorGUILayout.Space();
            _drawerSize = EditorGUILayout.IntField("Drawer size", _drawerSize);
            EditorGUILayout.Space();
